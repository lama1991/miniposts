    ...
    <table class="table table-bordered mb-3">
        ...
        @foreach($posts as $post)
            <tr>
                ...
                <td>{{ $post->title }}</td>
                <td>{{ $post->desc }}</td>
                <td>{{ $post->masked_content }}</td>
                <td>{{  $post->created_at->diffForHumans()}}</td>
                <td>added by {{ $post->user->name}}</td>
                @foreach($post->images as $image)
               <td> <img src="{{asset('storage/'.$image->link)}}" width="200" height="200"></td>
               

               @endforeach

                
                <td><a href="{{ url('/posts/' . $post->id) }}" title="View post"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> View</button></a></td>
                <td>  <a href="{{ url('/posts/' . $post->id . '/edit') }}" title="Edit post"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a></td>
